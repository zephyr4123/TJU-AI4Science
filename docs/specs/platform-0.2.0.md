...
| R-34 | 三层纲领（P-18，[#93](https://github.com/zephyr4123/TJU-AI4Science/issues/93) [#95](https://github.com/zephyr4123/TJU-AI4Science/issues/95) [#96](https://github.com/zephyr4123/TJU-AI4Science/issues/96) [#97](https://github.com/zephyr4123/TJU-AI4Science/issues/97) [#98](https://github.com/zephyr4123/TJU-AI4Science/issues/98) [#99](https://github.com/zephyr4123/TJU-AI4Science/issues/99)）：能力描述符换五栏（干什么 / 不干什么 / 要带什么进来 / 留下什么 / 什么时候停），inputs / outputs / criteria、`contracts/flow.py`、`show flow`、`GET /flow/check` 删；七颗能力收编成五颗（init 说清课题、design 写裁判跑基线、auto-research、analysis 写分析初稿、verify 核对数字）；流改成 `rooms`（房间、点名的能力与参数、断点），检查只查房间名 / 归属 / 参数 / 断点位置，出厂只留 `research`；`flow_state` 按房间记步；页面编辑台排房间挂能力插断点、脊柱一项一格；两份指南重写，「按钮」「键」换成命令 / 人确认；旧流一次性迁移脚本 `scripts/oneoff/migrate-flows-to-rooms.py`。**2026-09-18 完成** | 主人：能力之间的流检查按吃吐路径对表只能拼出一条线；重定义「文件即接口」；第一性原理——编排工作台是立规矩；能力细颗粒度、描述带专业术语；直白的工程语言 |
...
| 2026-09-18 | 加 R-34：三层纲领落地（[#93](https://github.com/zephyr4123/TJU-AI4Science/issues/93)） | 主人与 Claude 从「流通不通是怎么判的」一路问到第一性原理，把纲领敲定：房间任意组合、能力五栏、实现是下一层、断点是人确认 | 主人 + Claude |
