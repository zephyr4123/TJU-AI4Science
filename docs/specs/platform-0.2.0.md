...
| R-12 | 第一个真领域包 `domains/petab/`：profile、`prompts/experiment.md`、`skills/petab/SKILL.md`；skill 注入走 prompt 快照，实验与分析都吃；内仓 `docs/add-a-task.md`「十分钟接一个任务」（2026-09-19 改写成 `docs/start-a-workspace.md`）。**2026-09-16 完成**（[#39](https://github.com/zephyr4123/TJU-AI4Science/issues/39)） | packs §3，Q-2 |
...
| R-24 | 两块看板（[#58](https://github.com/zephyr4123/TJU-AI4Science/issues/58) 拟定，[#64](https://github.com/zephyr4123/TJU-AI4Science/issues/64) 落地）：编辑台改库（工作流墙、七段货架、拼流台，`POST /workflows` 存文件）、主页面改实例（对话 + 流程脊柱：当前 run 照的流一步一个模块，能力步骤按文件种类通用渲染，没 run 时是需求对齐，两颗键在脊柱上）；顶栏胶囊切换；三个页签废掉；设计系统第三版（五色 tokens、思源宋体 + IBM Plex、reactbits 只用在承担信息处）。**2026-09-17 完成** | 主人：页面不是固定流程，装什么流长什么样；配色样张主人认可「太顶级了」 |
...
| R-30 | 素材上 CDN（P-17，[#75](https://github.com/zephyr4123/TJU-AI4Science/issues/75) [#77](https://github.com/zephyr4123/TJU-AI4Science/issues/77) [#78](https://github.com/zephyr4123/TJU-AI4Science/issues/78)）：两段 MP4 与一张配图经本机处理推腾讯云 COS `ai4science/v1/`、`media.zephyrxiang.com` 分发（immutable 缓存、referer 白名单、预热）；页面 `assets.ts` 一处写 URL，`src/assets/` 二进制删；门口那一屏视频背景、欢迎屏配图 + GlassSurface 输入框（打字即开对话）、图标换 Phosphor；`ui-check` 守「`git ls-files ui/` 没有二进制」 | README P-17，workflow §5 |
| R-31 | 地方栏（[#79](https://github.com/zephyr4123/TJU-AI4Science/issues/79) [#80](https://github.com/zephyr4123/TJU-AI4Science/issues/80) [#81](https://github.com/zephyr4123/TJU-AI4Science/issues/81)）：工作区与编辑台是两个平行的世界——最左一条地方栏底下「工作区 / 编辑台」双向开关，上面工作区世界里列封面块与「新建」，编辑台世界里工作区块收掉（reactbits Dock 竖排），页眉只属于当前地方；对话抽屉入口做成带字的大按钮；门口那一屏一句话起工作区（文件夹名从标题推、封面卡随名字换）。**2026-09-18 完成** | 主人：编辑台不受工作区选择影响，两种选择别在页面上混在一起；历史对话入口谁都看不到；新建那屏太像表单 |
| R-32 | 脊柱改泳道、对话不绑流（[#82](https://github.com/zephyr4123/TJU-AI4Science/issues/82) [#83](https://github.com/zephyr4123/TJU-AI4Science/issues/83) [#84](https://github.com/zephyr4123/TJU-AI4Science/issues/84) [#85](https://github.com/zephyr4123/TJU-AI4Science/issues/85)）：`cap start` 记 `AI4SCI_CHAT_ID` 进 run、摘要带 `chat_id`；页面一块板几条泳道，默认收着只给目录，助理承接中的那条自动展开，有作业在跑时轮询；背景换风景（对话、脊柱、欢迎屏、库横幅、六张封面）。**2026-09-18 完成** | 主人：研究助理是流的观察者与执行者，一段对话可以同时开几条；默认只展示流的目录，承接了才展开；别用玻璃瓶、实验室味的图 |
| R-33 | 对话框上随时换模型与思考深度（[#86](https://github.com/zephyr4123/TJU-AI4Science/issues/86) [#87](https://github.com/zephyr4123/TJU-AI4Science/issues/87) [#88](https://github.com/zephyr4123/TJU-AI4Science/issues/88)）：`Chat` 端口加 `knobs()` 与每轮 `tuning`，Claude Code 适配器翻成 `--model` / `--effort`，对话 meta 记住、端点与 CLI 收；页面输入框两枚下拉片（reactbits GlideSelect）。**2026-09-18 完成** | 主人：每家底座 CLI 肯定都支持换模型和思考深度，先在适配器上暴露这一层，再同步输入框 UI；先后端单独验证再前后端闭环 |
| R-34 | 三层纲领（P-18，[#93](https://github.com/zephyr4123/TJU-AI4Science/issues/93) [#95](https://github.com/zephyr4123/TJU-AI4Science/issues/95) [#96](https://github.com/zephyr4123/TJU-AI4Science/issues/96) [#97](https://github.com/zephyr4123/TJU-AI4Science/issues/97) [#98](https://github.com/zephyr4123/TJU-AI4Science/issues/98) [#99](https://github.com/zephyr4123/TJU-AI4Science/issues/99)）：能力描述符换五栏（干什么 / 不干什么 / 要带什么进来 / 留下什么 / 什么时候停），inputs / outputs / criteria、`contracts/flow.py`、`show flow`、`GET /flow/check` 删；七颗能力收编成五颗（init 说清课题、design 写裁判跑基线、auto-research、analysis 写分析初稿、verify 核对数字）；流改成 `rooms`（阶段、点名的能力与参数、断点），检查只查阶段名 / 归属 / 参数 / 断点位置，出厂只留 `research`；`flow_state` 按阶段记步；页面编辑台排阶段挂能力插断点、脊柱一项一格；两份指南重写，「按钮」「键」换成命令 / 人确认；旧流一次性迁移脚本 `scripts/oneoff/migrate-flows-to-stages.py`。**2026-09-18 完成** | 主人：能力之间的流检查按吃吐路径对表只能拼出一条线；重定义「文件即接口」；第一性原理——编排工作台是立规矩；能力细颗粒度、描述带专业术语；直白的工程语言 |
| R-35 | 编辑台节点画布（[#100](https://github.com/zephyr4123/TJU-AI4Science/issues/100) [#101](https://github.com/zephyr4123/TJU-AI4Science/issues/101) [#102](https://github.com/zephyr4123/TJU-AI4Science/issues/102) [#103](https://github.com/zephyr4123/TJU-AI4Science/issues/103)）：React Flow 线性链，节点 = 研究阶段（能力 + 参数）/ 断点，画布铺满、阶段梯 + 玻璃题头、右上角库与保存、选中节点配置参数（`Param.in_flow` 分开每次调用才定的参数）、问题贴节点；造流助理改成右下角入口弹出的悬浮对话窗；全站文案清洗（短词、删解释性小标题、自造词出清、下划线填空线删）。**2026-09-19 完成** | 主人：先线性、一个节点一个阶段；用 reactbits 不手搓；短词不用谓宾串；下划线低劣；画布铺满、chat 悬浮 |
| R-36 | 工作区按阶段分目录、需求是根（P-19，[#104](https://github.com/zephyr4123/TJU-AI4Science/issues/104)）：`workspaces/<id>/` = `requirement.md` + `requirement.lock` + `materials/` + `flows/` + 七个阶段目录（英文 slug）+ `.ai4sci/`；每次产出一个子目录 `<stage>/<n>/`，`meta.yaml` 记 `from`（带 hash）、`by`、`flow` / `step`、`requirement`；没被引用没被签之前可改、之后冻住；断点 = 产出目录里的 `signed.json`，`publish.json` / `accept.json` / `flow.json` / `task/` / `runs/` / `init` 删；需求模板库 `templates/`；能力改成 `run(output_dir, inputs, ports, **params)` + `--from`，实验族的 schema 从 `contracts/` 搬进能力包；`framework/run/` 换成 `framework/workspace/`；CLI 加 `requirement confirm`、`sign <stage>/<n>`、`output new`、`show workspace|outputs|output|templates|template`；端点按需求 / 产出 / 签字重排；主页面两态（需求看板 markdown 渲染 + 确认；阶段格 + 产出 + 签）；三个样例工作区与旧数据一次性脚本迁移；文档只写现状 | 主人：现有目录为 auto-research 量身定做；科研多对多；需求形式开放、断点不能卡死、能力解耦；该删的大胆删 |
...
| 2026-09-18 | 加 R-34：三层纲领落地（[#93](https://github.com/zephyr4123/TJU-AI4Science/issues/93)） | 主人与 Claude 从「流通不通是怎么判的」一路问到第一性原理，把纲领敲定：阶段任意组合、能力五栏、实现是下一层、断点是人确认 | 主人 + Claude |
| 2026-09-19 | 加 R-35：编辑台节点画布（[#100](https://github.com/zephyr4123/TJU-AI4Science/issues/100)） | 主人拍板先线性、一个节点一个阶段；两轮看图挑刺：表单太 flat、下划线低劣、chat 要悬浮 | 主人 + Claude |
| 2026-09-19 | 加 R-36 工作区按阶段分目录、需求是根（[#104](https://github.com/zephyr4123/TJU-AI4Science/issues/104)） | 主人与 Claude 从第一性原理重定工作区目录：一个阶段一个目录、产出多对多、需求确认是唯一内置的门 | 主人 + Claude |
