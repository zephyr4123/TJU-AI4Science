...
- **第六版（2026-09-18，[#82](https://github.com/zephyr4123/TJU-AI4Science/issues/82)）**：脊柱改成一块板几条**泳道**——顶上需求对齐，每个 run 一条（照的那条流），取了没开跑的流一行「备着」；默认全部收着只给流的目录，只有助理正在承接的那条（作业在跑，或当前对话开的、还没验收）自动展开、动态渲染，人点哪条展开哪条；有作业在跑时每 10 秒重拉摘要。背景换风景：对话正文底下淡彩的云、脊柱底下层层山影，方格纸删；欢迎屏、库横幅、六张封面全换成风景（主人：别用玻璃瓶、实验室味的图）。
