...
- **Codex 适配器**（[#131](https://github.com/zephyr4123/TJU-AI4Science/issues/131)，2026-09-22 按官方文档 learn.chatgpt.com/docs/* 与 openai/codex rust-v0.147.0 源码对账、本机 codex-cli 0.147.0 逐条实测，全记在 `backends/codex.py` 文件头）：隔离的承重位是**私有 `CODEX_HOME`**（`~/.config/ai4sci/codex-home`，本机的 config.toml / plugins / MCP / hooks / memories / 个人 skills 全不进；`auth.json` 软链到真的 `~/.codex/auth.json`，凭据不复制；再带 `--ignore-user-config --ignore-rules`）；`$HOME/.agents/skills` 与随包的系统 skills 在私有 home 下照样被扫（一句 pong 19k tokens），只有 `[[skills.config]] path=<SKILL.md> enabled=false` 关得掉，每次起会话现扫现关（`-c skills.config=[...]`，关完 12k）；**沙箱是门**——`workspace-write` + `sandbox_workspace_write.writable_roots`（只许改的目录 + 平台自己要写的目录：数据根、按人的配置、uv 缓存，端口的 `runtime_paths`）+ `network_access=true`（`ai4sci` 要 ssh / pip / 下载），工作区外与 HOME 下的写「operation not permitted」，`.git` 只读；**没有按命令的白名单**（execpolicy 只管沙箱外、未命中即放行），端口的 `bash_rules` 只能写进 `tool_guide` 让它照做，真门仍是 `changed_files`；审批 exec 硬写 never；AGENTS.md 不读（`project_doc_max_bytes=0`）；联网 `web_search="live"`（服务端搜索，read-only 沙箱也通）；模型 `-m <slug>` + `-c model_reasoning_effort`（0.147 目录：gpt-5.6-sol / terra / luna、gpt-5.5，共有 low / medium / high / xhigh）；prompt 一律从 stdin 喂（位置参数 `-`，stdin 不关会一直等 EOF）；事件 `thread.started{thread_id}` / `turn.started` / `item.started|updated|completed{item}` / `turn.completed{usage}` / `turn.failed` / `error`，**没有逐字事件**（一段话说完才一条 agent_message，一段一条 text）；成本：ChatGPT 订阅报不出美元，`cost_usd` 一律 NaN、token 用量进 raw；退出码只有 0 / 1，没有轮数 / 花费的闸，超时是唯一的闸；长命令 agent 自己等（sleep 70 通过）。执行层 `--ephemeral`；协调层不带（续接要 rollout），`codex exec resume <thread_id> -` 续接，`developer_instructions` 塞指南但只在开线程那次生效。**嵌套会话**：助理 `--detach` 起的作业跑在上一层会话的沙箱 shell 里，作业里再起执行层 codex 是嵌套的（嵌套的 `sandbox-exec` 能起）；环境里的 `CODEX_HOME` 是我们给上一层的私有 home，照它算「真的 home」会把 auth.json 软链指向自己 → 401（演练第一次 `cap design` 就栽在这，`codex_home()` 现在指到自己的不算数）。
