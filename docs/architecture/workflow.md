...
- **第五版（2026-09-18，[#79](https://github.com/zephyr4123/TJU-AI4Science/issues/79)）**：最左一条**地方栏**：工作区一块块封面（靠近放大、点即切换）、末尾「新建」，隔一道空底下一块编辑台——编辑台是全局一个库、不在任何工作区里，所以不和工作区切换并排；页眉只属于当前地方（工作区标题 + 走到哪 + 封面色；编辑台注「不分工作区」；门口宽屏无页眉）；对话列表仍是抽屉，入口改成带字的大按钮「对话 · N」；门口那一屏改成一句话起工作区（写课题回车即建，文件夹名从标题推、可改，封面卡随名字换）。
