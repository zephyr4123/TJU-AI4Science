...
- **非交互 + 结构化输出**：Claude Code 走 `claude -p ... --output-format stream-json --verbose`，Codex 走 `codex exec --json`。
- **Codex 适配器**（[#131](https://github.com/zephyr4123/TJU-AI4Science/issues/131)，2026-09-22 按官方文档 learn.chatgpt.com/docs/* 与 openai/codex rust-v0.147.0 源码对账、本机 codex-cli 0.147.0 逐条实测，全记在 `backends/codex.py` 文件头）：隔离的承重位是**私有 `CODEX_HOME`**（`~/.config/ai4sci/codex-home/`，协调层用根、执行层用 `executor/` 子目录；本机的 config.toml / plugins / MCP / hooks / memories / 个人 skills 全不进；`auth.json` 软链到真的 `~/.codex/auth.json`，凭据不复制；再带 `--ignore-user-config`）；`$HOME/.agents/skills` 与随包的系统 skills 在私有 home 下照样被扫（一句 pong 19k tokens），只有 `[[skills.config]] path=<SKILL.md> enabled=false` 关得掉，每次起会话现扫现关（`-c skills.config=[...]`，关完 12k）；**`ai4sci` 在沙箱外跑、其余命令留在沙箱里**——Codex 没有按工具名的白名单，但 execpolicy 的 `.rules` 能做「这个前缀的命令在沙箱外跑」（`prefix_rule(decision="allow")`，实测 `/bin/zsh -lc 'ai4sci …'` 也命中），端口的 `bash_rules`（协调层 `ai4sci`，执行层 `ai4sci skill`）就翻成私有 home 的 `rules/ai4sci.rules`，与 Claude Code 的 `Bash(ai4sci *)` 一个模型：平台自己的 CLI 是放行的那扇门（能写平台的目录、能联网、起作业不嵌套沙箱），agent 敲的别的命令在沙箱里（`workspace-write`，可写根只有允许改的目录，没网，`.git` 只读）；嵌套的沙箱走不通（作业若在沙箱里起，里面的执行层 codex 连自己的可写根都写不进，实测 apply_patch「Operation not permitted」），这就是把 `ai4sci` 放到沙箱外的原因；审批 exec 硬写 never；AGENTS.md 不读（`project_doc_max_bytes=0`）；联网搜索 `web_search="live"`（服务端，沙箱没网也通）；模型 `-m <slug>` + `-c model_reasoning_effort`（0.147 目录：gpt-5.6-sol / terra / luna、gpt-5.5，共有 low / medium / high / xhigh）；prompt 一律从 stdin 喂（位置参数 `-`，stdin 不关会一直等 EOF）；事件 `thread.started{thread_id}` / `turn.started` / `item.started|updated|completed{item}` / `turn.completed{usage}` / `turn.failed` / `error`，**没有逐字事件**（一段话说完才一条 agent_message，一段一条 text）；成本：ChatGPT 订阅报不出美元，`cost_usd` 一律 NaN、token 用量进 raw；退出码只有 0 / 1，没有轮数 / 花费的闸，超时是唯一的闸；长命令 agent 自己等（sleep 70 通过）。执行层 `--ephemeral`；协调层不带（续接要 rollout），`codex exec resume <thread_id> -` 续接，`developer_instructions` 塞指南但只在开线程那次生效。环境里的 `CODEX_HOME` 指到私有根下面时不算「真的 home」（作业跑在上一层会话的 shell 里，照它算会把 auth.json 软链指向自己 → 401，演练第一次 `cap design` 栽在这）。
...
- **续接**：Claude Code 走 `claude -p <message> --resume <session id> --append-system-prompt <指南>`；隔离位与执行层同一组，但**不带** `--no-session-persistence`，多轮靠的就是 CLI 自己的会话持久化。Codex 走 `codex exec resume <thread_id> -`（resume 没有 `-C` / `-s` / `--add-dir` / `--color`，沙箱与可写根靠同一组 `-c`；`--ephemeral` 的线程续不了；resume 找不到线程会静默开一条新的，适配器对账 thread_id 不对就报错）。
- **指南怎么送到与「工具怎么用」**（P-25，[#131](https://github.com/zephyr4123/TJU-AI4Science/issues/131)）：端口的 `guide_channel` 说清指南每轮整份送（Claude Code `--append-system-prompt`，`"turn"`）还是只在开线程那次送（Codex `developer_instructions`，`"thread"`）——后者指南中途变了，框架把新指南全文塞进那一轮的话里（`conversation.GUIDE_REINJECT`），不然助理照旧指南办事。指南前言之后接这家 CLI 自己的「工具怎么用」（`Chat.tool_guide`：Claude Code 有 Read / Glob / Grep；Codex 只有 shell，看文件就是 ls / cat / rg 这类只读命令，沙箱管着写）——演练里 Codex 的助理照「只能运行 ai4sci」办，连 `materials/` 里有什么都不敢看，需求写成了「后续再读」。
...
- **词表**（P-21，[#112](https://github.com/zephyr4123/TJU-AI4Science/issues/112)）：页面、文档、指南一个概念一个词，没进词表的词不上屏；量词用「个 / 次 / 项」。
  ...
  | 助理 | 对话里的协调 agent：研究助理（项目页与工作区页，一个项目一位）、流程助理（编辑台） | 协调层、agent、模型 |
  ...
  | 按钮 | 保存、覆盖、排列、确认、打开对话、打开目录、新建项目、新建 | 整理、提交 |
...
- **项目**（[#136](https://github.com/zephyr4123/TJU-AI4Science/issues/136)，2026-09-22 页面落地）：页面先认项目。地方栏只剩首页 / 编辑台 / 设置三个键（主人：侧边栏列项目很鸡肋）；**首页是项目墙**——一格一个项目，字在前（名、目标一句、几个工作区、创建于哪天、几个在跑），封面压暗当底（主人：图可以作背景，信息必须陈列出来，图不能是主角），一个都没有就一句「还没有项目」加一枚「新建项目」；**门口那一屏**一句话起项目（目标一行可不填；第一个工作区跟助理说或在项目页「新建」）；**项目页**正中间一只对话输入框——一个项目只有一位助理，对话入口顺理成章只有这一个（主人：这就是做了项目之后带来的成果，像 Claude 的首页）——底下这个项目的工作区一行一个（状态一个词、走到第几步、产出几次）、「新建」展开的表单、「对话 · N」抽屉；第一句话开一段新对话、页面切成整屏的对话，「‹ 项目名」回来；**工作区页**页眉「‹ 项目名  工作区 ▾（换兄弟工作区）  看板 / 文件」，正文还是原来那套看板 / 文件，对话板在右、仍是项目的那一段。`GET /projects` 每行带 `goal` 与 `created_at`（目录的 birthtime；project.md 不塞日期）。
- **地方栏**：最左一条，三个键——首页、编辑台、设置，每个都带字；页眉只属于当前地方（项目页与工作区页底下铺项目的封面糊成一抹颜色）。对话列表是抽屉，入口带字「对话 · N」。
- **门口那一屏**：一句话起项目——写课题、目标一行可不填、回车即建；文件夹名从标题推、是内部 id 不显示也不让人填（[#109](https://github.com/zephyr4123/TJU-AI4Science/issues/109)）。工作区在项目页「新建」：一句话 + 选学科（需求提纲按学科起草），建好直接进那个工作区，需求还没确认。
- **工作区页**按 `requirement.lock` 在不在分两个状态。未确认——需求文档就是页面：助理按模板起草的 `requirement.md` 渲染成看板（文档里实际有的二级标题各一格，模板留的「待填」是空格子），右边对话，一个动作「确认需求」；页面只渲染不编辑，改需求只走对话（一个文件一个生产者，diff 才有意义）。已确认——需求收成顶部一条（版本、时间、点开侧滑看全文；助理又改了就显示 diff 与「确认下一版」），下面**一条流程一张表**：横向是流程经过的阶段（有什么阶段就几列，列头阶段名 + 能力），纵向是每一列跑过的每一次产出（编号 + 一个词），断点是两列之间一道线，右上角一句话说在等谁；产出点开侧滑看记录、文件、确认（[#107](https://github.com/zephyr4123/TJU-AI4Science/issues/107)）。主页面还有第二个**镜头**——页眉「看板 / 文件」切换，对话列两边都在（[#111](https://github.com/zephyr4123/TJU-AI4Science/issues/111)，主人：每个工作区要能看见盘上实际的目录，之后单独 Git 管理）：不是地方栏上第三个地方（地方栏按数据边界分），是同一个工作区的另一个镜头——看板答「做到哪了、在等谁」，文件答「盘上到底有什么」。左边一棵带平台语义的目录树（阶段目录写阶段名 + 图标、产出那一层编号 + 状态词 + 冻结锁、`.ai4sci/` 灰显、懒加载），右边按种类渲染；只看不改，改动走对话（手改会撞冻结）；Git 状态先不画。对话：研究者的话进气泡，助理逐字流出的 Markdown，工具调用一行一条原样显示（不折叠、不翻译、刷新后不消失）；输入框上两枚下拉片换模型与思考深度。
...
| 日期 | 改了什么 | 为什么 | 认可 |
|---|---|---|---|
| 2026-09-22 | §5 界面适配「项目」改成已落地：地方栏三键、首页项目墙、门口一句话起项目、项目页正中间的对话入口 + 工作区清单、工作区页页眉「‹ 项目名  工作区 ▾」；词表「主页面 / 编辑台」改「首页 / 项目页 / 工作区页 / 编辑台」、助理与看板两行同步（[#136](https://github.com/zephyr4123/TJU-AI4Science/issues/136)） | 主人：侧边栏列项目很鸡肋、图不能是主角、一个项目一位助理所以对话入口该在正中间 | 主人 + Claude |
