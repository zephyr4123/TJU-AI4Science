...
- **第五版（2026-09-18，[#79](https://github.com/zephyr4123/TJU-AI4Science/issues/79)）**：最左一条**地方栏**，先选世界再选世界里的东西：顶上「工作区 / 编辑台」两块是双向开关（两个平行的世界，各有自己的对话），工作区世界里再列一块块封面（靠近放大、点即切换）与「新建」，编辑台世界里工作区块整段收掉——编辑台是全局一个库，结构上就切不了工作区；页眉只属于当前地方（工作区标题 + 走到哪 + 封面色；编辑台注「不分工作区」；门口宽屏无页眉）；对话列表仍是抽屉，入口改成带字的大按钮「对话 · N」；门口那一屏改成一句话起工作区（写课题回车即建，文件夹名从标题推、可改，封面卡随名字换）。
