...
>
> Q-7 再记（2026-09-17 晚，[#63](https://github.com/zephyr4123/TJU-AI4Science/issues/63)）：异步作业做了——`--detach` 起作业、跑完框架以「框架」身份给对话发一轮叫醒 agent，run 记流与步序、在等谁现算。剩下的只有「人不在时怎么通知人」这一段通道（页面刷新能看到，主动推送没有），仍归 Q-7。
...
**2026-09-17 补记（[#58](https://github.com/zephyr4123/TJU-AI4Science/issues/58)）**：真实协调 agent 拼出并跑通自定义流之后（[#55](https://github.com/zephyr4123/TJU-AI4Science/issues/55)），几件事定了形：

- 等待状态的形状：run 记「照哪条流、走到第几步、停在等谁」，外加开跑时那条流的快照；页面读它高亮模块、亮钥匙。这是 Q-13 一直说的「实质」，现在有了明确的读取点。
- 两块看板：编辑台改库（对话拼流存文件，可选），主页面改实例（接流照跑，不立新流）；界线是可写目录，也就是工具白名单。
- 主页面右侧随工作流生成，一步一个模块，不写死流程；需求对齐是 intake 前三步，在主页面。
- 图的描述格式：还是 `workflows/*.yaml`，拟加步骤 `with:` 参数。谁解释图：主页面的协调 agent 读着按；零模型的运行器仍等画布。
- 拖拽画布仍不做；在编辑台「搓能力」是 v2，走代码的路（草稿子包 + 契约测试 + 人合并）。

**2026-09-18 补记（[#70](https://github.com/zephyr4123/TJU-AI4Science/issues/70)）**：图的实例化定了形——库里的流 `ai4sci flow take` 到工作区成实例，改参数与步骤都在实例上，run 快照实例（P-15）；解释图的仍是主页面的研究助理；造流归编辑台另一位助理，研究助理不造流（P-16）。

**2026-09-18 再记（[#93](https://github.com/zephyr4123/TJU-AI4Science/issues/93)，P-18）**：图的形状定了——不是带端口的积木，是**房间 + 断点**：`rooms` 列表里一项是一间房（可点名能力、带参数）或一个断点（停下来等人确认）。房间之间不接管子，机器只查房间名、能力归属、参数、断点位置；顺序与回头由人定。编辑台的拼流台就是排房间、挂能力、插断点；解释图的仍是主页面的研究助理，走到断点停下来等人说「继续」。「等谁拍板」机器可读了：便条 `flow.json` 记走到第几项，下一项是断点就是等人。
