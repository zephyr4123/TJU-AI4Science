...
```
 ┌──────────────────────────────────────────────────────────────────┐
 │ 协调层  人（研究者 / PI）+ 助理（协调 agent）                        │
 │   人     确认需求、在断点上签字、随时叫停；角色不可替代               │
 │   助理   持有需求与全局视角（看盘），选下一个阶段与能力、读结果、     │
 │          该问人时问人；一个项目一位助理                              │
 │   载体   一家 coding agent CLI 的多轮会话（Claude Code / Codex），   │
 │          由框架注入指南与前言；页面是它的脸                          │
 └──────┬─────────────────────────────────────────────────▲───────────┘
        │ 调 ai4sci CLI（每个动作一条子命令）                 │ 结论行、盘上状态、
        ▼                                                  │ 核对报告
 ┌──────────────────────────────────────────────────────────┴─────────┐
 │ 框架  诚实执行的基底，零模型调用，不做编排                              │
 │   能力   一个阶段里的一件活，各自可调，跑完即退                        │
 │   内环   跑实验、比分数、git 留或回滚、记账（实验族）                  │
 │   契约   需求确认、产出目录与签字、冻结 hash、流程文件、能力描述符      │
 │   判定   确定性判定：统计门、数字回溯、账本对 git（不调模型）          │
 └──────┬─────────────────────────────────────────────────▲───────────┘
        │ 每个能力一次调用：指令 + 点名的产出 + 领域约定 + skill 清单     │ 产物文件
        ▼                                                  │ 前后快照 diff
 ┌──────────────────────────────────────────────────────────┴─────────┐
 │ 执行层  coding agent CLI 子进程，唯一写代码的（Claude Code / Codex）  │
 │   写评分脚本、写基线、逐轮改代码、写分析；凡产出文件的活都在这         │
 │   每次新会话，无记忆，上下文从磁盘来（产出目录、账本、笔记）           │
 │   面前只有 ai4sci skill，其余命令不放行                              │
 └──────┬─────────────────────────────────────────────────────────────┘
        │ 调用
        ▼
 ┌────────────────────────────────────────────────────────────────────┐
 │ skill 脚本  确定性工具，换执行层能力不变                               │
 │   解析论文（pdf）· 拉材料（download）· 领域工具链的约定（petab）       │
 └────────────────────────────────────────────────────────────────────┘
```
