...
助理这一层 Codex 比上一轮的 Claude Code 快得多、省得多（8 轮合计 7 分钟出头，执行层 4 分钟写完壳），流程走法与 #120 那轮几乎一样；输掉的不是模型，是平台的四个坑（探测结果不带解释器路径、复现种子被平台换成 42–46、σ 让执行层自己算、continue 抹掉上一版基线）加一次定时关机。四个坑都修了根因（内仓 `codex-drill-2`）；Codex 自己的短板是撞墙不重探、凭上一轮的记忆答「做不了」，以及两次叫醒都没认出种子不对。
...
## 撞了什么坑、平台怎么修（都在内仓 `codex-drill-2`，每个有单测）
