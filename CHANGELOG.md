...
### 新增
- 公开区加 `research/reports/`：2026-09-23 汇报页（架构与实现现状、产出、开发环境与方法、分工草案、教程与接手），随 Pages 发布（#147）
...
- workflow §5 加删除边界、设置与自检、Codex 适配器实测；词表改「首页 / 项目页 / 工作区页 / 编辑台」（#131 #134 #136）
- README「五分钟承接」分只用与共同维护两种人（#138）
- 文档全面对齐代码：`CLAUDE.md` 装下协作与编码纲领，纲领与细则只留现状、每条 P-n 带机器判据，未决项只留真未决的，愿景重写，`domains.md` 搬进内仓，ADR-0003（#140）
...
### 变更
- 从 1.0.0 起承诺兼容，两仓大版本同日对齐；PR 只用 merge commit（#142）
- 内仓 2026-09-23 转公开；`repos.json` 与 README 的说法同步（#142）
- 0.2.0 spec 标已发布封存；research 选型页标封存并给被推翻的条目标「已被取代」（#140）

### 移除
- `docs/architecture/domains.md`（内容在内仓 `docs/add-a-domain.md`）；`assets/` 与两个空的 `.gitkeep`（#140 #142）
