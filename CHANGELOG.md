...
### 新增
- 会议纪要 `docs/meetings/2026-0923-division-of-labor.md`：分工草案（六个方向各自的目标、边界、入口文件、产出与验收；会上要定的七条；会后动作），拍板后再拆 issue
- README 改成整个仓库的地图：产品一眼看（Mermaid：库与项目、工作区、七个阶段的包含关系）+ 几个词的定义 + 文档地图（你要什么读哪份，三种人的阅读顺序）；纲领的四层 ASCII 图换成 Mermaid，§2 加库与实例的包含关系图、产出 `from` 引用的 DAG 示例；workflow.md 的实验内环 ASCII 图换成一轮的时序图；内仓同步（[#141](https://github.com/zephyr4123/TJU-AI4Science/issues/141)）
- 文档全面对齐代码（[#140](https://github.com/zephyr4123/TJU-AI4Science/issues/140)；主人 2026-09-23：结构好就只改内容、规矩按模块目录分层放、以代码为准做减法、盘出来的 bug 一并修）。**`CLAUDE.md` 装下纲领性的东西**：拓扑、红线（补「不加 AI 署名」）、协作方式（issue driven、spec coding、标签三轴、分支与合并前确认、做完必关、过程即收益）、写代码与验收的标准（三个清晰、第二个用例才抽象、重构彻底、质量不由写的人自证、未取证不下结论、直白工程语言）。**纲领 `docs/architecture/README.md` 全文按代码回写**：正文只留现状，每条 P-n 收成规则 + 机器判据 + 锚；四层图「工具」改「skill 脚本」、「裁判」改「确定性判定」；P-1 收窄、P-2 标明模型评审未实现、P-11 改「指南与领域 skill 隔离」、P-18 去掉「发布 / 验收是特例」、P-19 加「只有人能确认」的机器判据、P-20 文献主文件已定；§5 改成链接内仓文档；§6 加内仓一侧。**`workflow.md` 按代码回写**：清单类内容改为引用命令输出或代码，出厂件与数据根分两棵树，meta 字段补全，`run_N` / `loop resume` / `--run-id` / `analysis_v{n}` 这些旧写法清掉，验证写实际四项并标未实现的两条，词表补退役词与新词，历史压缩。`open-questions.md` 只留真未决的五项（Q-3 Q-4 Q-7 Q-12 Q-14），其余标已定。`vision.md` 只留目标、给谁用、凭什么、定位、不做、今天离它多远。`docs/architecture/domains.md` 删除（内容搬进内仓 `docs/add-a-domain.md`，只留有读取点的字段）。新 ADR-0003：外层追踪范围修订（案例卡可带小证据）与两仓发版差异。0.2.0 spec 标已发布封存；案例索引与六张案例卡的路径、命令名按现状改，旧卡加现名对照；research 索引与选型页标封存并给被推翻的条目标「已被取代」；README、scripts、Makefile、release.sh、md2html 的小错。两仓加 `AGENTS.md → CLAUDE.md` 符号链接（Codex 的入口），`.gitignore` 不再挡它
