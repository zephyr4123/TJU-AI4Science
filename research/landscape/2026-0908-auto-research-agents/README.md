...
>
> 怎么读：项目名在每一节首次出现处链接到[第 8 节](#8-开源项目现状详述)的条目，条目里有仓库、论文与数据集链接，并注明被哪些节引用；具体数字与判断的来源在脚注里，脚注可回跳正文。文献与项目的完整台账见同目录的 `references.md`。
...
| 阶段 | 目标 | 做什么 | 产出 |
|---|---|---|---|
| **0. 选型**（2–3 周） | 定底座与流水线 | 用同一个小任务分别跑 [AutoResearchClaw](#autoresearchclaw) 和 [InternAgent-1.5](#internagent)；底座各试 Claude Code 与一个国产模型 | 选型报告 + 成本对比 |
| **1. 模式 A 冷启动**（1–2 月） | 出第一批真实结果 | 选 1–2 个学院的指标型任务，套 [autoresearch](#karpathy-autoresearch) 循环；复现本院老师一篇论文作为入门任务（InternAgent 的 sci_tasks 做法） | 复现报告 + 若干指标提升 |
| **2. 学科 skill pack**（持续） | 学科适配 | 每学院 1 名研究者写 3–5 个 skill；把院内仿真/数据库包成 MCP | 开源 skill pack |
| **3. 验证层**（与 2 并行） | 单篇可信 | 实现[第 6.3 节](#63-具体怎么做六项检查每篇必过)的六项检查；评审用不同底座；复现在干净沙盒 | 验证工具链 |
| **4. 评测**（阶段 2 之后） | 框架可评 | 接入 [ResearchClawBench](#researchclawbench)；把工科任务投进去；按[第 7.4 节](#74-怎么做-ablation)做 ablation | 榜单结果 + ablation 论文 |
| **5. 模式 B/C 扩展** | 论文产出 + 算法进化 | 有数据集的学科上假设生成流水线；优化类问题上 [OpenEvolve](#openevolve-与-shinkaevolve) | 人机协作论文 |
